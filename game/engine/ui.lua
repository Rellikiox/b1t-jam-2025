...
function Node:get_mouse_position()
	return vec2 { love.mouse.getPosition() } - vec2 { self.x, self.y }
end
...
function Container:calculate_size()
	...
	elseif self.type == 'column' then
		...
		self.height =
			math.max(self.min_height, total_height + self.padding.top + self.padding.bottom)
...
function Paragraph:draw()
	...
	love.graphics.setColor(unpack(self.style.text_color))
	love.graphics.setFont(self.style.font)
...
function CenteredColumn:new(args)
	Column.new(
		self,
		{
			x = game_size.x / 2,
